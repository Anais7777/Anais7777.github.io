---
layout: post
title: "Delta Dunării"
maps_slug: delta-dunarii
categories: [ România ]
date: 2024-05-24
image: assets/images/DeltaDunarii/delta-dunarii.jpg
toc: true
beforetoc: "Cuprins"
trip:
  plecare: 2024-05-24
  intoarcere: 2024-05-26
  durata: 2 nopți și 3 zile
  persoane: 4
  temperatura: +16°C
  valuta: RON
  buget_actual: 12838.6
  buget_locala: 3210
  cazare: 3600.0
  cheltuieli_masina: 2392.8
  restaurante: 1520.0
  alimente: 168.4
  cumparaturi: 3157.4
  drum:
    summary: "Chișinău ↔ Delta Dunării"
    legs:
      - label: "Dus"
        date: "24 mai · plecare ~10:20"
        km: 400
        route: "Chișinău → Cahul → vamă → Galați → Murighiol"
        detail: "Ajuns la cazare ~17:00"
      - label: "Întors"
        date: "26 mai · plecare ~10:40"
        route: "Murighiol → Tulcea → bac Galați → Galați Mall → vamă → Chișinău"
        detail: "Ajuns acasă ~19:25"
    note: "Bac Galați: autoturism 20 RON + pasager 6 RON (șoferul nu plătește separat). Pe dus, la Galați treceam un pod aflat încă în construcție."
    note_icon: ship
---

## Cazare

Am stat la [Casa Vlad](https://www.booking.com/hotel/ro/casa-vlad-tulcea.ro.html) - Murighiol, Delta Dunării. Pentru 2 nopți și 3 zile am achitat **900 RON** (3 600 MDL).

## Buget

Delta Dunării, 4 persoane, 3 zile cu mașina: **3 210 RON** (12 839 MDL).

Cazare **900 RON** (3 600 MDL). Drum: rovinietă + asigurare **175 RON** (700 MDL), alimentări Rompetrol și Cahul **~397 RON** (1 589 MDL), bac Galați **26 RON** (104 MDL).

La La Sfatoi din Sfântu Gheorghe am mâncat pește proaspăt pentru **380 RON** (1 520 MDL) - șalău, saramură, scrumbie, somn prăjit cu mămăligă. Supermarket Murighiol **42 RON** (168 MDL). Cumpărături Dr Max, H&M, Carrefour **~790 RON** (3 157 MDL).

## Trip cu mașina

{% include trip-route.html %}

## Jurnal de călătorie

*Urmează să completez cu povestea mea.*
